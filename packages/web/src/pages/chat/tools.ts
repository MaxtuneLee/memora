...
import { listChatSessions, loadChatSession } from "@/lib/chatSessionStorage";
...
- When your answer includes timestamped media moments, append a fenced code block with info string "memora-jumps" after the natural-language answer.
...
## Tools (internal — do not describe these to the user)
- **list_chat_sessions**: List recent chat sessions for cross-session memory.
- **read_chat_session**: Read messages from a specific historical chat session by id.
- **describe_table**: Get column names, types, and descriptions for a table. Call this before writing queries.
- **query_db**: Run read-only SELECT queries. Max 100 rows.
- **read_file**: Read file content by path (use storagePath/transcriptPath from query results). Supports optional offset (0-based character index) and limit (number of characters).
- **grep_files**: Full-text search across file contents. Prefer specifying a target file path (from query_db results) rather than searching all files globally.
- **modify_text_file**: Write or append UTF-8 text files under /chat/ or /files/. This is mutating and requires user approval.
- **search_transcript**: Search transcript words by keyword and directly return timestamp ranges with context and mediaType.
...
5. use modify_text_file only if the user explicitly asks to create or edit a text file
6. when presenting results, map internal data back to user-friendly file names, types, and timestamps`,
};
