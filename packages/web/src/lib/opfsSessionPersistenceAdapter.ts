  ...
  return {
    save: async (agentId, key, data) => {
      if (key === MEMORY_KEY) {
        ...
        const personalityValue = nextMemory[PERSONALITY_KEY];
        const personalityText = normalizePersonalityText(
          typeof personalityValue === "string" ? personalityValue : "",
        );
        if (personalityText) {
          await savePersonalityDoc(personalityText);
        }
        return;
      }
    ...
    grep: async (agentId, pattern) => {
      ...
      const derivedMemory = globalMemory ?? (
        fallbackPersonality
          ? { [PERSONALITY_KEY]: fallbackPersonality }
          : null
      );
