import {
  ...
  type LocalModelRequestEnvelope,
  type LocalModelSequencedEventEnvelope,
  type LocalModelSharedWorkerMessage,
  type LocalModelTask,
...
import {
  ...
  type StoredModelWorkerEvent,
  writeModelWorkerSnapshotState,
...
export const startSharedModelWorkerRuntime = (
  ...
  const enqueuePersist = (request: RequestState): Promise<void> => {
    request.persistChain = request.persistChain
      .catch(() => undefined)
      .then(() => writeModelWorkerSnapshotState(pool, request.snapshot));
  ...
  const dispatch = (): void => {
    ...
    void runTask(queued.task, {
      ...
      .then(() => {
        if (!isTerminalStatus(request.snapshot.events.at(-1)?.event ?? { type: "chat-complete" })) {
          emit(request, {
            type: "status",
  ...
  const handleMessage = async (
    ...
    switch (message.type) {
      ...
      case "cancel": {
        ...
        if (
          !request ||
          isTerminalStatus(request.snapshot.events.at(-1)?.event ?? { type: "chat-complete" })
        ) {
          return;
        }
      ...
      case "acknowledge": {
        ...
        await removeModelWorkerSnapshot(pool, message.requestId);
        return;
      }
      case "disconnect":
        disconnect(port);
  ...
  setWorkerDebugReporter((message) => {
    for (const port of ports) port.postMessage(message);
  });

  const scope = self as unknown as SharedWorkerConnectScope;
