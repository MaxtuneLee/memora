...
2. Settle the destination before designing anything — a Home Grid widget (saved to the user's home, where it lands in a `1 × 1` square of roughly `280–336px`) or a chat-only widget (full column width, auto-fitting its height). If the request does not make it obvious, ask one short question and wait for the answer. A request that sounds like a dashboard is not automatically a full-size panel.
3. Read exactly one primary module guideline from `guidelines/`.
4. Read the required section files for that module from `sections/`.
5. For a Home Grid widget, lay out inside the square first and only then decide what fits in it — see "Home Grid sizing" in `README.md`.
6. Only then call `show_widget`.
